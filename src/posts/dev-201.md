...
To fade between content we apply an `active` class on the nav we want to show. We're transition the `opacity` from 0 to 1.
...
different nav sections, but you could capture these values dynamically on page load as well.

<pre><code class="prism language-js line-numbers">// Hardcoded size and positions for each nav sectio
const dimensions = {
...
.popover {
  transform-origin: center -100px; /* Axis moved up to create larger arc of movement. */
